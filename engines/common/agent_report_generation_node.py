...
class AgentReportGenerationNode(ResearchNode):
    ...
    async def __call__(self, state: ResearchGraphState[Any]) -> dict[str, Any]:
        ...
        report_context = json.dumps([{"title": section["title"], "body": section.get("body", "")} for section in sections],
                                    ensure_ascii=False)
    ...
    async def _generate_report(self, report_context: str, query: str) -> str:
        ...
        user_prompt = prompt_template.format(research_topic=query,report_context=report_context)
        raw_text = await self.ctx.llm_client.generate_text(
