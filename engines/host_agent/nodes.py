...
from engines.host_agent.discussion_events import build_agent_discussion_event, build_judgement_discussion_event
...
class HostNodes:
    ...
    def __init__(self, section_judge: HostSectionJudge):
        self._section_judge = section_judge

    async def collect_agent_output(self, state: HostState) -> dict[str, Any]:
        """保存当前Agent章节输出并生成对应讨论事件"""
        agent_output: AgentSectionOutput = AgentSectionOutput.from_section_ready_event(state['event_payload'])
        state['section_pair_store'].store_output(agent_output)
        discussion_events = [build_agent_discussion_event(state['task_id'], agent_output)]
        host_name = role_display_name('host')
        logger.info(f"[{host_name}]收到章节就绪事件"
    ...
    async def generate_section_judgement(self, state: HostState) -> dict[str, Any]:
        """调用Host llm 为当前已齐备章节生成结构化研判"""
        section_pair = state['section_pair_store'].get_ready_pair()
        host_name = role_display_name('host')
        logger.info(f"[{host_name}]双Agent章节已齐备 开始研判"
                    f"章节={section_pair.section_key}")
        section_judgement: SectionJudgement = await self._section_judge.judge_section(section_pair)
        return {"section_judgement": section_judgement}

    async def apply_section_judgement(self, state: HostState) -> dict[str, Any]:
        """将章节研判应用到Host状态并生成讨论事件"""
    ...
    async def save_judgements(self, state: HostState) -> dict[str, Any]:
        ...
        json_path = save_report(output_dir,"judgements.json",structured_content)
