...
from engines.common.events import EventType, subscribe, unsubscribe, publish, publish_host_discussion_message
...
class SectionReadyListener:
    """串行消费章节就绪事件并驱动Host LangGraph"""

    def __init__(self):
        self._graph = build_graph(HostSectionJudge())
        self._host_states: dict[str, HostState] = {}  # task_id 主持人状态
    ...
    def start(self):
        ...
        logger.info("SectionReadyListener: 已启动Host 章节研判")
    ...
    def _enqueue_event(self, _event_type: EventType, event_payload: dict[str, Any]):
        """将章节就绪事件加入串行消费队列"""
        self._event_queue.put_nowait(event_payload)

    async def _process_events(self):
        """持续消费章节就绪事件并发布Host讨论事件"""
        while True:
            # 1. 从队列中获取数据包
