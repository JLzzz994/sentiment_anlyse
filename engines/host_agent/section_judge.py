...
from engines.prompts.host import HOST_SECTION_JUDGE_USER_PROMPT, HOST_SECTION_JUDGE_SYSTEM_PROMPT


class HostSectionJudge:
    """调用Host llm 对 Insight 与 Media 的同章节输出进行研判"""
    llm_client: LLMClient = LLMClient.from_role("host")

    async def judge_section(self,section_pair:AgentSectionPair)->SectionJudgement:
        """生成单章节结构化研判"""
    ...
    def _build_judgement_context(self, section_pair:AgentSectionPair)->str:
        """组装当前双Agent的同章节输出"""
        judgement_context = {
            "section":{
