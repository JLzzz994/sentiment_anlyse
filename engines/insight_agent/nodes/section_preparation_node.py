...
class SectionPreparationNode(ResearchNode):
    ...
    async def __call__(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        self.ctx.report_progress("planning", f"{agent_name} 开始初始化固定章节并为各章节选择、排序和截取证据", 30)
        ...
        self.ctx.report_progress("planning", f"{agent_name} 完成初始化固定章节并为各章节选择、排序和截取证据", 40)
...
def _select_section_records(section_key: str, records_by_id: Mapping[str, EvidenceRecord],
    ...
    matched_records: list[EvidenceRecord] = [records_by_id[record_id] for record_id in
                                             section_record_ids.get(section_key, [])]
    # todo 冗余排序
