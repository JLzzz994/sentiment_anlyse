"""BGE-M3 向量编码器: 负责Dense 与Sparse 向量的生成"""
...
def _normalize_sparse_vector(sparse_vector: dict) -> dict[int, float]:
    return {int(token_id): float(weight) for token_id, weight in sparse_vector.items()}
...
class VectorEmbedder:
    ...
    def encode(self, texts: Iterable[str]) -> list[tuple[list[float], dict[int, float]]]:
        """
        生成稠密与稀疏向量组合
        ...
        model_output = self.model.encode(
            sentences=text_items,
            return_dense=True,
            return_sparse=True

        )
        return [(dense.tolist(), _normalize_sparse_vector(sparse)) for dense, sparse in
                zip(model_output["dense_vecs"], model_output["lexical_weights"])]


if __name__ == '__main__':
    texts = ["今天天气真好","今天不适合学习"]
    embedder = VectorEmbedder()
    results = embedder.encode(texts)
    for idx,(dense_vec,sparse_vec) in enumerate(results, start=1):
        print(f"内容: {texts[idx-1]}")
        print(f"稠密向量维度:{len(dense_vec)}")
        print(f"稠密向量前5个元素:{dense_vec[:5]}")
        print(f"稀疏向量词数: {len(sparse_vec)}")
        # 打印稀疏向量前3歌token_id 及权重
