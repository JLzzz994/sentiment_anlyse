...
class SearchPlaningNode(ResearchNode):
    ...
    async def __call__(self, state: MediaState) -> dict[str, Any]:
        ...
        self.ctx.report_progress(
            "planning",
            f"{agent_name} 开始规划公域搜索策略",
            10,
        )

        logger.info(f"{agent_name} 开始规划公域搜索策略, 研究主题:{state['query']}")
        ...
        self.ctx.report_progress(
            "planning",
            f"{agent_name} 公域搜索策略规划完成",
            20,
        )
        logger.info(f"{agent_name} 完成规划公域搜索策略")
    ...
    async def _generate_search_plan(self, research_topic: str, ) -> MediaSearchPlanOutput:
        ...
        """
        # 工具名字和描述
        search_tools = [{"name": tool, "description": description} for tool, description in
                        SEARCH_TOOL_DESCRIPTIONS.items()]
