...
from engines.common.research_graph_runtime import ProgressCallback, ResearchRunContext, handle_research_graph
from engines.contracts.agent_roles import RoleKey
from engines.media_agent.graph import build_graph


async def media_agent_handler(
        role:RoleKey,
        query:str,
        task_id:str,
    ...
    await handle_research_graph(ctx=ctx,graph=build_graph(ctx),query=query)
